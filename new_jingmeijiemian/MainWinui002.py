...
class QUnFrameWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.initTitleLabel() # 安放标题栏标签  #初始化一个标签控件  001这里搞个标签QTitleLabel放标题文字wanghui2
        self.setWindowTitle = self._setTitleText(self.setWindowTitle) # 用装饰器将设置WindowTitle名字函数共享到标题栏标签上
        #self._TitleLabel.setText("wanghui2")  #设置标签名称
        self.setWindowTitle("wanghui2")
        self.initLayout() # 设置框架布局  002#这里又搞个标签，防止上面一个标签被覆盖
    ...
    def initTitleLabel(self):
        ...
        self._TitleLabel.setIndent(100) # 设置标题栏文本缩进，距离左边框的距离
        ...
        self._TitleLabel.setStyleSheet(
            '''QTitleLabel{background:#F76677;border-radius:5px;} QTitleLabel:hover{background:red;}''')#标签设置为红色
    def initLayout(self):
        # 设置框架布局
        self._MainLayout = QVBoxLayout()#垂直布局
        self._MainLayout.setSpacing(0)#QGridLayout 里面有两个函数分别是setmargin和setspacing其中setmargin是设置总的外围边框，setspacing是设置间隔的
        self.Label001=QLabel()
        self.Label001.setObjectName('Label002')
        self.Label001.setStyleSheet(
            '''QLabel{background:green;border-radius:5px;} QLabel:hover{background:red;}''')  # 标签设置为绿色
        self._MainLayout.addWidget(self.Label001, Qt.AlignLeft) # 顶一个QLabel在竖放框架第一行，以免正常内容挤占到标题范围里
        #self._MainLayout.addWidget(QLabel(), Qt.AlignLeft)  # 顶一个QLabel在竖放框架第一行，以免正常内容挤占到标题范围里
        self._MainLayout.addStretch()#添加了一个伸缩空间,应该添加到布局的底部，就是向上顶
        self.setLayout(self._MainLayout)#初始化垂直布局
    ...
    def _setTitleText(self, func):
        # 设置标题栏标签的装饰器函数，
        def wrapper(*args):
            self._TitleLabel.setText(*args)#在设置窗体标题前，同时设置了标签标题，本质是把设置窗口标题函数，变成及设置窗体标题也设置标签标题，多是一句，哎
            return func(*args)  #设置窗体标题
        return wrapper
